...
infoLabel.Text = "ℹ️ Auto Collect:\n\n✅ Auto scan items\n✅ Smart teleport\n✅ Return to spawn"
...
local function getDropOffLocation()
    -- Cari spawn location dari workspace
    ...
    -- Alternatif: cari dari folder spawns
    ...
    -- Fallback: ambil posisi karakter saat ini (spawn awal)
    local character = player.Character
    if character and character:FindFirstChild("HumanoidRootPart") then
...
local function autoCollectItems()
    isRunning = true
    updateStatus("Scanning...", Color3.fromRGB(100, 200, 100))
    wait(0.5)
    
    -- PENTING: Sesuaikan nama item sesuai game Rainbow Friends
    -- Cek nama exact item di Explorer workspace!
    local itemFolders = {
    ...
    local targetItemKeywords = {
        "block", "fuse", "battery", "food", "item", "collectible"
    }
    
    local items = {}
    local foundItems = {}
    
    -- Scan di folder-folder yang mungkin
        ...
        if folder then
            for _, obj in pairs(folder:GetDescendants()) do
                if obj:IsA("BasePart") or obj:IsA("Model") then
                    ...
                    -- Cek apakah nama mengandung keyword item
                    for _, keyword in pairs(targetItemKeywords) do
                        if string.find(objName, keyword) and not foundItems[obj] then
                            -- Pastikan objnya bukan bagian dari map/building
                            if obj:IsA("BasePart") and obj.Size.Y < 15 and obj.Size.X < 15 then
                                table.insert(items, obj)
                                foundItems[obj] = true
                                break
                            elseif obj:IsA("Model") and obj.PrimaryPart then
                                table.insert(items, obj.PrimaryPart)
                                foundItems[obj] = true
                                break
                            end
                        end
    ...
    -- Jika tidak menemukan di folder, scan workspace langsung (lebih hati-hati)
    if #items == 0 then
        updateStatus("Deep scanning...", Color3.fromRGB(255, 200, 100))
        ...
        for _, obj in pairs(workspace:GetChildren()) do
            if (obj:IsA("BasePart") or obj:IsA("Model")) and not obj:IsA("Terrain") then
                ...
                for _, keyword in pairs(targetItemKeywords) do
                    if string.find(objName, keyword) and not foundItems[obj] then
                        if obj:IsA("BasePart") and obj.Size.Y < 10 then
                            table.insert(items, obj)
                            foundItems[obj] = true
                            break
                        elseif obj:IsA("Model") and obj.PrimaryPart then
    ...
    for i, item in pairs(items) do
        ...
        if item and item.Parent then
            -- Validasi posisi tidak di luar map (Y position reasonable)
            if item.Position.Y > -50 and item.Position.Y < 500 then
                teleportTo(item.Position)
                collected = collected + 1
                updateStatus("Collecting " .. collected .. "/" .. #items, Color3.fromRGB(255, 200, 100))
                wait(0.4) -- Kasih waktu item ke-collect
            end
        end
    end
    
    -- Kembali ke drop-off (spawn location)
    if autoCollectEnabled and collected > 0 then
        wait(0.5)
        local dropOff = getDropOffLocation()
        updateStatus("Returning to spawn...", Color3.fromRGB(100, 200, 255))
        teleportTo(dropOff)
        wait(0.5)
        updateStatus("Complete! ✅ (" .. collected .. " items)", Color3.fromRGB(100, 255, 100))
        wait(2)
    end
